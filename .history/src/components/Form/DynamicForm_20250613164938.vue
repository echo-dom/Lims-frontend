<template>
  <el-form :model="formData" ref="formRef" label-width="150px" class="space-y-4">
    <template v-for="field in templateData.fields">
      <el-form-item :key="field.id" :label="field.label" :prop="field.name" :rules="field.required ? [{ required: true, message: field.label + ' 为必填', trigger: 'blur' }] : []">
