          ...
          <template #dropdown>
            <el-dropdown-menu class="column-selector">
              <div class="dropdown-header">
                <el-checkbox v-model="selectAllColumns" :indeterminate="isIndeterminate" @change="handleSelectAll">
                  列显示配置
                </el-checkbox>
              </div>
        ...
        >
          ...
          >
            ...
            <template #default="scope">
              <div
                class="cell-wrapper"
                :class="{ editing: isEditing(scope.row, col.prop) }"
                style="width:100%; height:100%; display:flex; justify-content:center; align-items:center;"
              >
                <!-- 如果不可编辑或非当前编辑单元格，展示文本，若可编辑则可点击 -->
                <template v-if="!col.editable || !isEditing(scope.row, col.prop)">
                  <span
                    v-if="col.editable"
                    class="editable-cell"
                    style="cursor: pointer; width:100%; display:inline-block; text-align:center;"
                    @click="() => startEdit(scope.row, col)"
                  >
                    <!-- 根据类型格式化展示 -->
                    <template v-if="col.type === 'date'">
                      {{ scope.row[col.prop] ? formatDateByYMD(scope.row[col.prop], 'yyyy-MM-dd') : '' }}
                    </template>
                    <template v-else>
                      {{ scope.row[col.prop] }}
                    </template>
                  </span>
                  <span v-else style="width:100%; display:inline-block; text-align:center;">
                    <template v-if="col.type === 'date'">
                      {{ scope.row[col.prop] ? formatDateByYMD(scope.row[col.prop], 'yyyy-MM-dd') : '' }}
                    </template>
                    <template v-else>
                ...
                <!-- 编辑状态：根据 editType 或 col.type 渲染对应编辑组件 -->
                <template v-else>
                  <!-- 文本编辑 -->
                  <el-input
                    v-if="!col.editType || col.editType === 'text'"
                    v-model="editCache[getRowKey(scope.row)][col.prop]"
                    size="small"
                    class="edit-input"
                    :ref="setInputRef(getRowKey(scope.row), col.prop)"
                    @blur="() => saveEdit(scope.row, col)"
                    @keyup.enter.native="() => saveEdit(scope.row, col)"
                    :data-row-key="getRowKey(scope.row)"
                    :data-prop="col.prop"
                    style="width: 100%;"
                  />
                  <!-- 数字编辑 -->
                  ...
                  <!-- 下拉 select 编辑 -->
                  <el-select
                    v-else-if="col.editType === 'select' && col.options"
                    v-model="editCache[getRowKey(scope.row)][col.prop]"
                    size="small"
                    @change="() => saveEdit(scope.row, col)"
                    style="width: 100%;"
...
.cell-wrapper.editing {
  background-color: #f0f9eb; /* 编辑高亮背景，可自定义 */
}
.editable-cell:hover {
