            ...
            <el-card shadow="hover">
                ...
                <el-form :rules="rules" :inline="true" :model="formData" class="demo-form-inline">
                    <el-form-item label="选择客户" required>
                        <el-select v-model="formData.customer"  filterable remote reserve-keyword placeholder="请选择客户" prop="customer"
                            remote-show-suffix :remote-method="remoteMethod" :loading="loading2" @change="changeCustomer" style="width: 380px" >
                            <el-option v-for="item in options" :key="item.compcode" :label="item.label"
                                :value="item.compcode" />
                        </el-select>
                    </el-form-item>
                     <el-form-item label="地址编码" required>
                         <el-input v-model="formData.ccode" />
                    </el-form-item>
                    <el-form-item label="要求完成时间" required>
                        <el-date-picker v-model="formData.inspectioncompletiondate" type="date" placeholder="Pick a date" clearable />
                    </el-form-item>
                </el-form>
            </el-card>
        </div>
