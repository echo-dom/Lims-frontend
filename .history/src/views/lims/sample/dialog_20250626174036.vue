                ...
                <el-form :inline="true" :model="formData" class="demo-form-inline">
                    <el-form-item label="选择客户">
                        <el-select v-model="formData.customer"  filterable remote reserve-keyword placeholder="请选择客户"
                            remote-show-suffix :remote-method="remoteMethod" :loading="loading2" @change="changeCustomer" >
...
const changeCustomer = (val)=>{
    const code = list.value.filter(ite=> ite.compcode == val)
    if(!code.trim()){
        ElMessage.warning('该客户无地址编码,请手动输入！')
    }
    formData.value.ccode = code
}
const submit = ()=>{
