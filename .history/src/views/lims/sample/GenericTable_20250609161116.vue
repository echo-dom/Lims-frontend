    ...
    <el-container class="container">
        <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
            <el-tab-pane v-for="tab in tabs" :key="tab.name" :name="tab.name" :label="tab.label">
                <el-button v-for="button in tab.buttons" :key="button.label" @click="button.handler" class="btn">{{
                    button.label }}</el-button>
            </el-tab-pane>
        </el-tabs>
        <el-config-provider :locale="locale">
            <el-table @selection-change="handleSelectionChange" ref="multipleTableRef" v-loading.lock="loading"
                id="mtable" @row-click="hangdleRowClick" highlight-current-row
                ...
                <el-table-column v-for="cloumn in columnList" :key="cloumn.prop" align="center" show-overflow-tooltip
                    ...
                    <template #header>
                        <!-- 添加一个 div 包裹表头文本，并添加样式 -->
                        <div class="header-text" title="{{ cloumn.label }}">{{ cloumn.label }}</div>
