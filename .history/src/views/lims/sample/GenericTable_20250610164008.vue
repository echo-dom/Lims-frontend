...
/* 1. 让 header th 相对定位，并预留右侧空间（宽度可根据实际微调） */
:deep(.el-table__header th) {
  position: relative;
  padding-right: 60px;
}

/* 2. 列标题 padding，避免和图标挤在一起 */
.column-title {
...
/* 4. 过滤触发器垂直居中定位 */
:deep(.el-table__header th .el-table__column-filter-trigger) {
  position: absolute;
  top: 50%;
  right: 32px;
  transform: translateY(-50%);
...
/* 5. （可选）header-inner 布局，如果你在 header slot 包了多个元素 */
.header-inner {
  display: flex;
  flex-direction: column;
}
